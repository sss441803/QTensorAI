...
def circ2buckets(qubit_count, circuit, pdict={}, max_depth=None):
    ...
    # the circuit
    measurement_circ = [[ops.M(qubit, n_batch=n_batch) for qubit in range(qubit_count)]]
    ...
    op = ops.M(0, n_batch=n_batch)  # create a single measurement gate object
...
class M(ParallelGate):
    ...
    def gen_tensor(self):
        ...
        #                    [0, 1]
        #                    ]).repeat(n_batch,1,1)
        return torch.tensor([[1, 0], [0, 1]]).repeat(n_batch, 1, 1)
...
class Z(ParallelGate):
    ...
    def gen_tensor(self):
        n_batch = self.parameters['n_batch']
        return torch.tensor([1, -1]).repeat(n_batch,1)
...
class YPhase(ParallelParametricGate):
    ...
    def _gen_tensor(**parameters):
        ...
        c = torch.cos(np.pi * alpha / 2).unsqueeze(1)[:,None]*torch.tensor([[1,0],[0,1]])
        s = torch.sin(np.pi * alpha / 2).unsqueeze(1)[:,None]*torch.tensor([[0, -1],[1,0]])
...
class XPhase(ParallelParametricGate):
    ...
    def _gen_tensor(**parameters):
        ...
        c = torch.cos(np.pi*alpha/2).unsqueeze(1)[:,None]*torch.tensor([[1,0],[0,1]])
        s = torch.sin(np.pi*alpha/2).unsqueeze(1)[:,None]*torch.tensor([[0, -1j], [-1j, 0]])
        g = torch_j_exp(1j*np.pi*alpha/2).unsqueeze(1)[:,None]
        return g*c + g*s
...
class ParallelTorchQkernelComposer(QkernelComposer):
    ...
    def cost_operator(self):
        ...
        for qubit in self.qubits:
            self.apply_gate(self.operators.Z, qubit, n_batch=self.n_batch)
